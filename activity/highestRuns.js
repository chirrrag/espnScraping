...
function cb(error, response, data) {
    if(error) {
        ...
        for(let i = 0; i < tables.length; i++) {
            ...
            for(let j = 0; j < teamBatsmen.length; j++) {
                ...
                if(eachBatCol.length == 8) {
                    ...
                    let runs = cheerio(eachBatCol[2]).text().trim();
                    console.log(playerName, "        ", runs);

                    if(highestRuns <= Number(runs)) {
                        highestRuns = runs;
                        playerNameWithHighestRuns = playerName;
                    }
                }
            }
            console.log("--------------------------");
        ...
        console.log("Player", playerNameWithHighestRuns, "Score is", highestRuns);
